local QBCore = exports['qb-core']:GetCoreObject()
local PlayerData = QBCore.Functions.GetPlayerData() -- Just for resource restart (same as event handler)
local radioMenu = false
local isRadioOn = false
local RadioChannel = 0
local RadioVolume = 50
local hasRadio = false
local radioProp = nil

--Function
local function LoadAnimDic(dict)
    if not HasAnimDictLoaded(dict) then
        RequestAnimDict(dict)
        while not HasAnimDictLoaded(dict) do
            Wait(0)
        end
    end
end

local function connectToRadio(channel)
    RadioChannel = channel

    if isRadioOn then -- Disconnect from previously joined channel
        exports["pma-voice"]:setRadioChannel(0)
    else
        isRadioOn = true
        exports["pma-voice"]:setVoiceProperty("radioEnabled", true)
    end

    exports["pma-voice"]:setRadioChannel(channel)
    QBCore.Functions.Notify(Lang:t('joined_to_radio', { channel = channel .. '.00 MHz' }), 'success')
    TriggerServerEvent('qb-radio:joinRadio', channel)
end

local function closeEvent()
    TriggerEvent("InteractSound_CL:PlayOnOne", "click", 0.6)
end

local function leaveRadio()
    closeEvent()
    RadioChannel = 0
    isRadioOn = false
    exports["pma-voice"]:setRadioChannel(0)
    exports["pma-voice"]:setVoiceProperty("radioEnabled", false)
    QBCore.Functions.Notify(Lang:t('you_leave'), 'error')
    TriggerServerEvent('qb-radio:leaveRadio')
end

local function toggleRadioAnimation(pState)
    LoadAnimDic("cellphone@")
    if pState then
        TriggerEvent("attachItemRadio", "radio01")
        TaskPlayAnim(PlayerPedId(), "cellphone@", "cellphone_text_read_base", 2.0, 3.0, -1, 49, 0, 0, 0, 0)
        radioProp = CreateObject(`prop_cs_hand_radio`, 1.0, 1.0, 1.0, 1, 1, 0)
        AttachEntityToEntity(radioProp, PlayerPedId(), GetPedBoneIndex(PlayerPedId(), 57005), 0.14, 0.01, -0.02, 110.0,
            120.0, -15.0, 1, 0, 0, 0, 2, 1)
    else
        StopAnimTask(PlayerPedId(), "cellphone@", "cellphone_text_read_base", 1.0)
        ClearPedTasks(PlayerPedId())
        if radioProp ~= 0 then
            DeleteObject(radioProp)
            radioProp = 0
        end
    end
end

local function toggleRadio(toggle)
    radioMenu = toggle
    SetNuiFocus(radioMenu, radioMenu)
    if radioMenu then
        toggleRadioAnimation(true)
        SendNUIMessage({ type = "open" })
    else
        toggleRadioAnimation(false)
        SendNUIMessage({ type = "close" })
    end
end

local function doRadioCheck(PlayerItems)
    for _, item in pairs(PlayerItems) do
        if item.name == "radio" then
            hasRadio = true
            break;
        end
    end
end

--Exports
exports("IsRadioOn", function()
    return isRadioOn
end)

--Events

-- Handles state right when the player selects their character and location.
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    PlayerData = QBCore.Functions.GetPlayerData()
    doRadioCheck(PlayerData.items)
end)

-- Resets state on logout, in case of character change.
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
    doRadioCheck({})
    PlayerData = {}
    leaveRadio()
end)

-- Handles state when PlayerData is changed. We're just looking for inventory updates.
RegisterNetEvent('QBCore:Player:SetPlayerData', function(val)
    PlayerData = val
    doRadioCheck(PlayerData.items)
end)

-- Handles state if resource is restarted live.
AddEventHandler('onResourceStart', function(resource)
    if GetCurrentResourceName() == resource then
        PlayerData = QBCore.Functions.GetPlayerData()
        doRadioCheck(PlayerData.items)
    end
end)

RegisterNetEvent('qb-radio:use', function()
    toggleRadio(not radioMenu)
end)

RegisterNetEvent('qb-radio:isRadioOnDrop', function()
    if RadioChannel ~= 0 then
        leaveRadio()
    end
end)

-- NUI
RegisterNUICallback('joinRadio', function(data, cb)
    local rchannel = tonumber(data.channel)
    if rchannel ~= nil then
        if rchannel <= Config.MaxFrequency and rchannel ~= 0 then
            if rchannel ~= RadioChannel then
                if Config.RestrictedChannels[rchannel] ~= nil then
                    if Config.RestrictedChannels[rchannel][PlayerData.job.name] and PlayerData.job.onduty then
                        connectToRadio(rchannel)
                        cb({ channel = rchannel })
                    else
                        QBCore.Functions.Notify(Lang:t('restricted_channel_error'), 'error')
                    end
                else
                    connectToRadio(rchannel)
                    cb({ channel = rchannel })
                end
            else
                QBCore.Functions.Notify(Lang:t('you_on_radio'), 'error')
            end
        else
            QBCore.Functions.Notify(Lang:t('invalid_radio'), 'error')
        end
    else
        QBCore.Functions.Notify(Lang:t('invalid_radio'), 'error')
    end
end)

RegisterNUICallback('leaveRadio', function(_, cb)
    if RadioChannel == 0 then
        QBCore.Functions.Notify(Lang:t('not_on_radio'), 'error')
    else
        leaveRadio()
        cb({ status = "success" })
    end
end)

RegisterNUICallback("volumeUp", function(_, cb)
    if RadioVolume <= 95 then
        RadioVolume = RadioVolume + 5
        exports["pma-voice"]:setRadioVolume(RadioVolume)
        QBCore.Functions.Notify(Lang:t("radio_volume", { value = RadioVolume }), "success")
        cb({ status = "success" })
    else
        QBCore.Functions.Notify(Lang:t("max_radio_volume"), "error")
    end
end)

RegisterNUICallback("volumeDown", function(_, cb)
    if RadioVolume >= 10 then
        RadioVolume = RadioVolume - 5
        exports["pma-voice"]:setRadioVolume(RadioVolume)
        QBCore.Functions.Notify(Lang:t("volume_radio", { value = RadioVolume }), "success")
        cb({ status = "success" })
    else
        QBCore.Functions.Notify(Lang:t("min_radio_volume"), "error")
    end
    cb('ok')
end)

RegisterNUICallback("increaseradiochannel", function(_, cb)
    if not isRadioOn then return end

    local newChannel = RadioChannel + 1

    if newChannel < Config.MaxFrequency then
        connectToRadio(newChannel)
        QBCore.Functions.Notify(Lang:t("radio_channel", { value = newChannel }), "success")
        cb({ channel = newChannel })
    end
end)

RegisterNUICallback("decreaseradiochannel", function(_, cb)
    if not isRadioOn then return end

    local newChannel = RadioChannel - 1

    if newChannel >= 1 then
        connectToRadio(newChannel)
        QBCore.Functions.Notify(Lang:t("radio_channel", { value = newChannel }), "success")
        cb({ channel = newChannel })
    end
end)

RegisterNUICallback('poweredOff', function(_, cb)
    leaveRadio()
    cb({ status = 'success' })
end)

RegisterNUICallback('escape', function(_, cb)
    toggleRadio(false)
    cb({ status = 'success' })
end)

--Main Thread
CreateThread(function()
    while true do
        Wait(1000)
        if LocalPlayer.state.isLoggedIn and isRadioOn then
            if not hasRadio or PlayerData.metadata.isdead or PlayerData.metadata.inlaststand then
                if RadioChannel ~= 0 then
                    leaveRadio()
                end
            end
        end
    end
end)
